...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    io::stdin().read_line(&mut count_input).expect("입력을 읽을 수 없습니다");
    ...
    println!("   ✅ 성공: {}개", put_success);
    println!("   ❌ 실패: {}개", put_errors);
    println!("   📈 성공률: {:.1}%", if put_success + put_errors > 0 { (put_success as f64 / (put_success + put_errors) as f64) * 100.0 } else { 0.0 });
    
    println!("\n🔍 GET 결과:");
    println!("   ✅ 성공: {}개", get_success);
    println!("   ❌ 실패: {}개", get_errors);
    println!("   📈 성공률: {:.1}%", if get_success + get_errors > 0 { (get_success as f64 / (get_success + get_errors) as f64) * 100.0 } else { 0.0 });
